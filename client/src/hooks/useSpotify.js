 ...
 *
 * Polls Spotify REST API for currently-playing track, progress, and user profile.
 * Interpolates progress locally between polls so the bar moves smoothly.
 *
 * Exports:
...
// ── fmtMs ────────────────────────────────────────────────────
/** Format milliseconds → "m:ss"  e.g. 97400 → "1:37" */
...
export function useSpotify(accessToken, onTokenExpired) {
  ...
  useEffect(() => {
    ...
    fetch("https://api.spotify.com/v1/me", { headers: authHeaders() })
      ...
      .then(data => {
        if (!data) return;
        setProfile({
          name:      data.display_name || "Listener",
  ...
  useEffect(() => {
    ...
    const handle401 = () => {
      console.warn("[useSpotify] 401 — token expired");
      onTokenExpired?.();
    };
    ...
    const fetchTrack = async () => {
      try {
        ...
        if (res.status === 204) {
          setTrack(null);
          setIsPlaying(false);
          lastPollRef.current = { progressMs: 0, ts: Date.now(), playing: false };
          return;
        }
        if (!res.ok) return;
        ...
        console.error("[useSpotify] fetch error:", err);
      }
    };
    ...
    // Smooth progress — tick every 500 ms between polls
    interpolatorRef.current = setInterval(() => {
      const { progressMs: base, ts, playing } = lastPollRef.current;
      if (!playing) return;
      setProgressMs(prev => {
        const elapsed  = Date.now() - ts;
        const inferred = base + elapsed;
        // Only advance, never jump backwards
        return inferred > prev ? inferred : prev;
      });
    }, 500);
